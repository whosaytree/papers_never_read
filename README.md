...
- 技术分享：5 篇正式记录
- 代码项目：4 个正式项目
- 关键图：119 篇论文已挂载关键图或关键表
- 详细笔记：2 篇论文已挂载独立 Markdown 笔记页
- 一级分类：8 个
...
项目级协作规则见 [AGENTS.md](AGENTS.md)。论文、技术分享和代码项目统一采用“草稿 → 多轮讨论/修改 → 确认入库 → 发布”流程。

直接发送以下任一句即可获取 Markdown 草稿：
...
支持中英文冒号、空格及 Blog/博客/技术分享/Vlog 等别名。只发链接、题目或笼统要求添加资料，也默认先出草稿。草稿会标明版本和待审状态；讨论和修改期间保持待审，不写入正式数据、不构建、不提交、不推送。

审核完当前草稿后，回复 **“确认入库”** 即可，也可说“按这版发布”。无需固定长句。“好的”“可以”“确认”或确认单个字段不构成发布授权。多个待审条目需明确批准范围；未展示草稿时先出草稿供审核。审批细则以 AGENTS.md 为准，下面保留各类内容的字段与发布步骤。
...
只有已展示草稿，并收到用户针对该草稿的明确入库批准（推荐回复“确认入库”），才进入入库发布；遵循上方统一审核规则。
...
使用 `预入库blog：链接` 获取草稿，审核完回复“确认入库”；遵循上方统一审核规则。
...
使用 `预入库代码：链接` 获取草稿，审核完回复“确认入库”；遵循上方统一审核规则。
...
| 2026-05-28 | 4 | 3 | 3 | 10 |
| 2026-05-29 | 1 | 1 | 1 | 3 |
| 2026-05-30 | 11 | 10 | 7 | 28 |
| 2026-05-31 | 25 | 10 | 13 | 48 |
